...
#define AXSC_NUM_ERROR_MSG  1

/**
* List of error messages
" @see basic_error_messages
*/
const char*   error_messages[AXSC_NUM_ERROR_MSG] = { "Unknown error" };
...
AXSC_API unsigned int axsc_get( const int query_state ) {
  switch( query_state )
  {
    ...
    case AXSC_LIBRARY_ID:
      return( state.lib_id );

    case AXSC_LAST_ERROR:
      {
...
AXSC_API const char* axsc_get_error_message( const unsigned int error_number ) {
  int ok = AXE_IN_RANGE( error_number, 0, (AXE_NUM_BASIC_ERROR_MSG + AXSC_NUM_ERROR_MSG - 1) );
...
/* $Id: axsc_script.cpp,v 1.2 2004/08/29 18:28:44 doneval Exp $ */
