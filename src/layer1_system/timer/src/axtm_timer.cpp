...
#define AXTM_NUM_ERROR_MSG  1

/**
* List of error messages
" @see basic_error_messages
*/
const char*   error_messages[AXTM_NUM_ERROR_MSG] = { "Unknown error" };
...
AXTM_API unsigned int axtm_get( const int query_state ) {
  switch( query_state )
  {
    ...
    case AXTM_LIBRARY_ID:
      return( state.lib_id );

    case AXTM_LAST_ERROR:
      {
...
AXTM_API const char* axtm_get_error_message( const unsigned int error_number ) {
  int ok = AXE_IN_RANGE( error_number, 0, (AXE_NUM_BASIC_ERROR_MSG + AXTM_NUM_ERROR_MSG - 1) );
...
/* $Id: axtm_timer.cpp,v 1.3 2004/07/27 22:42:48 doneval Exp $ */
